...
static const char *shellText[] =
{
    [SHELL_TEXT_INFO] =
        ENTER
        " _         _   _                  _          _ _ "ENTER
        "| |    ___| |_| |_ ___ _ __   ___| |__   ___| | |"ENTER
        "| |   / _ \\ __| __/ _ \\ '__| / __| '_ \\ / _ \\ | |"ENTER
        "| |__|  __/ |_| ||  __/ |    \\__ \\ | | |  __/ | |"ENTER
        "|_____\\___|\\__|\\__\\___|_|    |___/_| |_|\\___|_|_|"ENTER
        ENTER
        "Build:       "__DATE__" "__TIME__ ENTER
        "Version:     "SHELL_VERSION ENTER
        "Copyright:   (c) 2021 GuoYi XueYang"ENTER,
    [SHELL_TEXT_CMD_TOO_LONG] = 
        ENTER"Warning: Command is too long"ENTER,
    [SHELL_TEXT_CMD_LIST] = 
        ENTER"Command List:"ENTER,
    [SHELL_TEXT_VAR_LIST] = 
        ENTER"Var List:"ENTER,
    [SHELL_TEXT_USER_LIST] = 
        ENTER"User List:"ENTER,
    [SHELL_TEXT_KEY_LIST] =
        ENTER"Key List:"ENTER,
    [SHELL_TEXT_CMD_NOT_FOUND] = 
        "Command \"%s\" not Found"ENTER,
    [SHELL_TEXT_POINT_CANNOT_MODIFY] = 
        "can't set pointer"ENTER,
    ...
    [SHELL_TEXT_PASSWORD_HINT] = 
        ENTER"Please input password:",
    [SHELL_TEXT_PASSWORD_ERROR] = 
        ENTER"passwrod error"ENTER,
...
static void shellHandleInit(Shell *shell, char *buffer, unsigned short size);
static int shellExrOneCmd(Shell *shell, int argc, char *argv[]);
static void shellPrintCmdNotFound(Shell *shell, const char *cmd);

static void shellPrintCmdNotFound(Shell *shell, const char *cmd)
{
    shellPrint(shell, shellText[SHELL_TEXT_CMD_NOT_FOUND], cmd);
}
 ...
 * @param shell shell对象
 */
void shellInit(Shell *shell, char *buffer, unsigned short size, int mode)
{
#ifdef __linux__
    terminal_echo_enable(STDIN_FILENO, 0);
#endif
    shellHandleInit(shell, buffer, size);
    ...
    if (mode == 0) {    /* 不使用单命令模式 */
        shellSetUser(shell, shellSeekCommand(shell,
                                             ...
                                             0));
        shellWriteString(shell, ENTER ENTER);
...
static void shellWriteCommandLine(Shell *shell)
{
    ...
    {
        // checkshellWriteString(shell, ENTER);
        shellPrint(shell, shell->info.user->data.user.name);
        shellPrint(shell, ":/$ ");
    }
    else
    {
                ...
                && shell->parser.buffer[i] != 0))
        {
            ...
            if (record == 1)
            {
                if (shell->parser.paramCount >= SHELL_PARAMETER_MAX_NUMBER) {
                    shellPrint(shell, ENTER"warning : shell命令参数(包含命令名在内)超过最大数量(%d), "
                                      "超出的部分将被忽略.", SHELL_PARAMETER_MAX_NUMBER);
                    break;
                }
                               ...
                               unsigned short compareLength)
{
    ...
    unsigned short count = shell->commandList.count -
        ((ADDR_T)base - (ADDR_T)shell->commandList.base) / sizeof(ShellCommand);
 ...
 * @param command 命令
 */
static int shellRunCommand(Shell *shell, ShellCommand *command)
{
...
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0), SHELL_KEY_VALUE_UP, shellUp, up);
...
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0), SHELL_KEY_VALUE_DOWN, shellDown, down);
...
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0), SHELL_KEY_VALUE_TAB, shellTab, tab);
...
void shellEnter(Shell *shell)
{
    ...
    if (shell->status.isChecked)
    {
        ...
        {
            shellPrintCmdNotFound(shell, shell->parser.param[0]);
        }
    }
    else
    {
...
void shellHelp(int argc, char *argv[])
{
    ...
    else if (argc > 1)
    {
        ShellCommand *command = shellSeekCommand(shell,
                                                 ...
                                                 0);
        if (command == NULL) {
            shellPrintCmdNotFound(shell, argv[1]);
        } else {
...
{
    argc = argc - 1;
    if (argc > SHELL_PARAMETER_MAX_NUMBER) {
        shellPrint(shell, ENTER"warning : shell命令参数(包含命令名在内)超过最大数量(%d), "
                          "超出的部分将被忽略.",  SHELL_PARAMETER_MAX_NUMBER);
    }

    int count = argc < SHELL_PARAMETER_MAX_NUMBER ? argc : SHELL_PARAMETER_MAX_NUMBER;
    shell->parser.paramCount = 0;
    for (size_t i = 0; i < count; i++) {
        shell->parser.param[shell->parser.paramCount ++] = argv[i + 1];
    }
    ...
    if (command != NULL)
    {
        returnValue = shellRunCommand(shell, command);
    }
    else
    {
        shellPrintCmdNotFound(shell, shell->parser.param[0]);
 ...
 * 
 */
int shellTask(void *param, int argc, char *argv[])
{
    Shell *shell = (Shell *)param;
    int retval = EXIT_SUCCESS;

    if (USE_ONE_CMD_MODE(argc)) {  
        /* 使用单命令模式 */
        retval = shellExrOneCmd(shell, argc, argv);
    } else {
        char data;
        shell->status.running = 1;
...
#endif
        shell->status.running = 0;
    }

#ifdef __linux__
    terminal_echo_enable(STDIN_FILENO, 1);
#endif
